  ...
  reader.onload = (event) => {
    ...
    img.onload = async () => {
      ...
      canvas.toBlob(async (blob) => {
        ...
        try {
          const response = await fetch('https://phonecall.onrender.com/upload', {
            method: 'POST',
            body: formData
          });
...
document.getElementById('schedule-button').addEventListener('click', async () => {
  ...
  try {
    const response = await fetch('https://phonecall.onrender.com/schedule', {
      method: 'POST',
      headers: {
...
const fetchAndDisplayBalance = async () => {
  try {
    const response = await fetch('https://phonecall.onrender.com/api/balance');
